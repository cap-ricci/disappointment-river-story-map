function writeStory(json, data) {
    ...
    var start =
        ...
        '" data-lat="' +
        first_feature.geometry.coordinates[0] +
        '" data-zoom="4" data-animation="flyto"><h2>' +
        data.title +
        "</h2>" +
        data.storymap_img +
        "<p>" +
    ...
    features.forEach(function (e, idx) {
        ...
        chapter.append($("<h2>" + title + "</h2>" + "<img data-title=" + title + " src=" + prop.img + ">"));
  ...
  function story_init(name) {
    ...
    fetch("http://localhost:8080/geoserver/explore/wfs?service=WFS&" + "version=1.1.0" + "&request=GetFeature" + "&typename=explore:" + element.points_name + "&outputFormat=application/json", {
        method: "POST",
  ...
  function noshow_storymap() {
    ...
    $(".options").empty();
    $("#story").empty();
    toggleLines(true, ['cook', 'hearne', 'pond', 'mackenzieLife', 'mackenzie', 'castner', 'mackay']);
  }
  
  //travel lines are in an independent layer
  function load_lines(list) {
    var lines = [];
        ...
        if (Object.hasOwnProperty.call(storylines[key], "lines_name")) {
            ...
            lines[key] = new ol.layer.Vector({
                ...
                source: new ol.source.Vector({
                    url: "http://localhost:8080/geoserver/explore/wfs?service=WFS&" + "version=1.1.0" + "&request=GetFeature" + "&typename=explore:" + element.lines_name + "&outputFormat=application/json",
